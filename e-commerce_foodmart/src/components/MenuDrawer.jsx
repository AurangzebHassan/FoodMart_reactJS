...
export default function MobileMenuDrawer({
  ...
  useEffect(() => {
    if (openMenuDrawer) {
      ...
      // ✅ CHANGED: added slight delay for smooth slide-in (same as CartDrawer)
      setTimeout(() => setIsVisible(true), 10);
    } else {
      // when drawer closes, start slide-out animation
      setIsVisible(false);

      // ✅ CHANGED: restore body scroll AFTER animation finishes
      const t = setTimeout(() => {
  ...
  const handleClose = () => {
    // ✅ CHANGED: same logic as CartDrawer
  ...
  return (
      ...
      {/* same transform/transition classes as CartDrawer */}
        ...
        {/* Vertical menu links */}
              ...
              <ul className="space-y-4 mb-6 p-6">
          {["Women", "Men", "Kids", "Accessories"].map((item) => (
            <li key={item}>
              <button
                onClick={() => navigateTo(`/${item.toLowerCase()}`)}
                className="w-full text-left px-2 py-3 rounded hover:bg-gray-100"
              >
                {item}
              </button>
            </li>
        ...
        {/* Pages select */}
              ...
              <div className="mb-6 p-6">
          <label htmlFor="mobile-pages" className="block font-medium mb-2">
            Pages
          </label>
          <select
            id="mobile-pages"
