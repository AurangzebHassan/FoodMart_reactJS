...
import { useCart } from "../context/CartContext"; // ✅ Use CartContext for real-time cart state

import { useAuth } from "../context/AuthContext"; // to get current logged-in user
...
export default function CartDrawer({ onClose })
{
    ...
    } = useCart(); // get real-time cart state and methods
    ...
    // ------------------- Slide-in Animation and Scroll Lock -------------------
        ...
        useEffect(() => 
        {
            ...
            // slight delay for smooth slide-in animation
    ...
    // ------------------ Close Drawer -------------------
        ...
        const handleClose = () => 
        {
            ...
            setTimeout(() => onClose(), 200); // wait for animation to finish before removing from DOM
        };
    ...
    // ------------------- Calculate Totals -------------------
        ...
        // Derived values from CartContext (already reactive)
  
        const totalQuantity = cartQuantity;
  
        const totalBill = cartTotal;
    ...
    >
      ...
      >
        ...
        {/* ------------------- Header ------------------- */}
          ...
          <span className={`text-white text-[25px] hover:-translate-y-1 bg-yellow-500 ${totalQuantity > 9 ? "px-3" : "px-4.5"} rounded-full font-bold transition-all duration-200`}>
            {totalQuantity} {/* total quantity from CartContext */}
          </span>
            </div>
        ...
        {/* ------------------- Cart Items ------------------- */}
          ...
          {cartItems.map((item) => {
            const product = item.product; // each cartItem already enriched in CartContext
                ...
                <div className="flex-1">
                  ...
                  <div className="flex justify-between mt-10">
                    <div className="flex gap-2">
                      <p className="text-gray-600 text-md font-mono">
                    ...
                    <p className="text-gray-700 font-mono">
                      {formatPrice(product.price * item.quantity, "USD")}
                    </p>
                  </div>
                </div>
              </div>
        ...
        {/* ------------------- Footer / Checkout ------------------- */}
            ...
            <div className="mt-3 border-t-3 pt-5">
                <p className="flex justify-between font-extrabold text-orange-500">
                    ...
                    <span className="text-[25px]">{formatPrice(totalBill, "USD")}</span>
                </p>
                {/* <button
                ...
                <button
                    ...
                    onClick={async () => {
                        ...
                        await clearCart();           // CartContext method
                        setCheckoutLoading(false);  // stop loading
                        alert("Checkout successful! Cart cleared.");
                    }}
                    disabled={checkoutLoading}   // disable while loading
                    >
